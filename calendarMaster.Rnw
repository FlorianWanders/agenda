...
# Remove year from column names, except for first month [-1] and for Januaries
colnames(overview)[-1][!grepl("January", names(overview)[-1])] <- gsub(
  '([A-z]+) .*', 
  '\\1', 
  colnames(overview)[-1])[!grepl("January", names(overview)[-1])]

## Weekends in overview
wknds <- apply(overview, 1:2, function(x) grepl("(Sat)|(Sun)", x))

## Mondays in overview (for adding weekly numbers)
